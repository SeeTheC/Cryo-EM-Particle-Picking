...
function [layers,options,minInputDim] = createRCNNArch(noOfClass,checkpointBasePath)    
    minInputDim=[32 32 3];
...
function [options] = createRCNNOption(checkpointBasePath)
    checkpointBasePath=strcat(checkpointBasePath,'/checkpoint');
    mkdir(checkpointBasePath);
    % Options for step 1.
