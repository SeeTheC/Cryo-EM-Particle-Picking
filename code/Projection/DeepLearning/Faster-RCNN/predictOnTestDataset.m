function [avgPrecision,result,tblPrecsionRecall] = predictOnTestDataset(detector,minDim,testData,downsample,saveModelPath)
    ...
    for i = 1:height(testData)      
        ...
        I = imread(filename);   
        if(downsample>1)
            I=imresize(I,1/downsample);
            testData.box{i}=testData.box{i}./downsample;
        end
        if(size(I,1)<minDim(1))
            %I=imresize(I,ceil(minDim(1)/size(I,1)));
            fprintf('Error:Dim of Image is less than required [%d,%d] dim:[%d,%d]\n',minDim(1),minDim(2),size(I,1),size(I,2))
            continue;
        end  
        if(size(I,2)<minDim(2))
            I=imresize(I,ceil(minDim(2)/size(I,2)));
            fprintf('Error:Dim of Image is less than required [%d,%d] dim:[%d,%d]\n',minDim(1),minDim(2),size(I,1),size(I,2))
            continue;
        end  
        ...
        if size(bboxes,1) ==0
            ...
            fprintf(negfid,'NO Particle FOUND:%s/%s\n',sfn{end-1},sfn{end});            
        end
    end  
    fclose(negfid);
