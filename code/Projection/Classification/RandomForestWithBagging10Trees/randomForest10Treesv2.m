...
function [ status ] = randomForest10Treesv2(server,noOfScales)
    %% INIT        
    status='fail';                
    if server
    ...
    basepath=strcat(basepath,'/_data-Y,Zv.10'); 
...
function generate(modelNumber,basepath)
    ...
    savepath = strcat(basepath,'/rf10/','/model-',num2str(modelNumber));
    mkdir(savepath);
    %trainFile= strcat(datapath,'/train_set1.txt');
    %testFile= strcat(datapath,'/test_set1.txt');
    fid = fopen(strcat(savepath,'/rf10_info.txt'), 'w+');
    ...
    rf10Model = TreeBagger(10,trainX,trainY, ...
                    'ClassNames',{'-1','1'});
                
    %% Save Trained Model
    save(strcat(savepath,'/rf10Model.mat'),'rf10Model');
    clear rf10Model;
    %clear compactSVMModel;
    %% Load Trained Model
    struct=load(strcat(savepath,'/rf10Model.mat'));
    RF10Model=struct.rf10Model;
    whos('RF10Model')


    %% validate test
    [predLabelCell,PostProbs] = predict(RF10Model,validateX);
    ...
    [predLabelCell,PostProbs] = predict(RF10Model,testX);
